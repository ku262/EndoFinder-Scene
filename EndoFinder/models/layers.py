...
class Attention(nn.Module):
    ...
    def forward(self, x, z=None):
        if z is None:
            ...
            q = self.to_q(x) #[B, 196, 768]
            k, v = self.to_kv(z).chunk(2, dim=-1) #[B, N*Patch_num, 768]
            qkv = (q, k, v)

        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = self.heads), qkv)

        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale #[B*N, h, 2048, 768/h]
        ...
        out = rearrange(out, 'b h n d -> b n (h d)') #[B*N, 2048, 768]
